...
if(isset($_GET['q']) && $_GET['q']){
	...
	UguisuMailList::setSearchWord($_GET['type'],"contain",$_GET['q']);
}

//フィルタ
if(isset($_GET['filter']) && isset($ACCOUNT[$account]['filter'][$_GET['filter']])){
	$filterRecord = $ACCOUNT[$account]['filter'][$_GET['filter']];
	UguisuMailList::setSearchWord($filterRecord['key'],$filterRecord['mode'],$filterRecord['value']);
...
<a href="../write/?account=<?php echo $account; ?>" target="_blank" title="メールを作成" onClick="popUp('../write/?account=<?php echo $account; ?>'); return false;">作成</a>

<select name="categoryTo" id="categoryTo" onChange="showMoveLinks();">
<option value="" style="color:#777;">(移動先)</option>
<?php
foreach(UguisuAccount::getCategoryList() as $categoryId => $categoryName){
	echo "<option value=\"".$categoryId."\">".$categoryName."</option>\n";
}
?>
</select>
<a href="mail-list-thread.php?<?php echo $entityGetQuery; ?>" title="スレッド表示">スレ</a> 
<?php
if($category==$CONFIG['trashBox']){
	echo "<a href=\"utility.php?mode=archiveTrash&account=".$account."\" target=\"paneBottom\" ".
	"onClick=\"return confirm('ごみ箱の中のメールをアーカイブデータベースに移動します。\\nこの処理には時間がかかります。\\nタイムアウトが発生した場合は、処理が中断されてますので再度実行してください。\\nよろしいですか？');\" title=\"ごみ箱をアーカイブ\">アーカイブ</a> \n";
	echo "<a href=\"utility.php?mode=emptyTrash&account=".$account."\" target=\"paneBottom\" ".
	"onClick=\"return confirm('ごみ箱の中のメールを完全に削除します。\\nよろしいですか？');\" title=\"ごみ箱を空にする\">ごみ空</a> \n";
}else{
	echo "<a href=\"utility.php?mode=autoTrash&account=".$account."&category=".$category."\" ".
	"onClick=\"return confirm('古いメールをごみ箱に移動します。\\nよろしいですか？');\" target=\"paneBottom\" onClick=\"return false;\" title=\"過去メールをごみ箱に移動\">ごみ移</a> \n";
}
?>
